...
from core import Database, DayRepository, DayState, QuotaService, TemplateRepository
...
class CalendarView(Vertical):
    ...
    def watch_month(self, old: date, new: date) -> None:  # type: ignore[override]
        ...
        self.checked_days = self.app.service.get_checked_days(month_start, month_end)  # type: ignore[attr-defined]
        self.refresh_calendar()
...
class CheckModal(ModalScreen[Optional[Dict]]):
    ...
    def compose(self) -> ComposeResult:
        yield Static(
            f"{self.item['item_text']} (importance {self.item['importance']})",
            classes="modal-title",
        )
...
class SettingsScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        template_json = json.dumps(self.service.load_template(), indent=2)
        yield Header(show_clock=False)
        yield Vertical(
            Label(
                "Edit template JSON (list of items with id, text, importance, subitems)"
            ),
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "reset":
            try:
                default_template = self.service.reset_template()
                self.query_one("#template-editor", TextArea).value = json.dumps(
                    default_template, indent=2
                )
        ...
        if event.button.id == "save":
            ...
            try:
                ...
                self.service.save_template(parsed)
...
class MainScreen(Screen):
    ...
    def load_day(self, day: date) -> None:
        self.state = self.service.get_day(day)
        day_label = self.query_one("#day-label", Label)
        day_label.update(
            f"Selected: {day.isoformat()}" + (" (Closed)" if self.state.closed else "")
        )
        ...
        for row in self.state.items:
            items_container.mount(QuotaItemView(row, self.state.closed))
        cal = self.query_one(CalendarView)
        month_start = cal.month.replace(day=1)
        next_month = (month_start.replace(day=28) + timedelta(days=4)).replace(day=1)
        month_end = next_month - timedelta(days=1)
        cal.set_checked_days(self.service.get_checked_days(month_start, month_end))
    ...
    def on_quota_clicked(self, message: QuotaClicked) -> None:
        if self.state and self.state.closed:
            ...
            return
        item = next(
            (i for i in (self.state.items if self.state else []) if i["item_id"] == message.item_id),
            None,
        )
        if not item:
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "settings":
            self.app.push_screen(SettingsScreen(self.service))
        elif event.button.id == "close-day":
            self.service.close_day(self.selected_day)
            self.load_day(self.selected_day)
        elif event.button.id == "reopen-day":
            self.service.reopen_day(self.selected_day)
            self.load_day(self.selected_day)
...
class QuotaApp(App):
    ...
    def __init__(self) -> None:
        ...
        self.service = QuotaService(day_repo, template_repo)
